...
import { 
  ...
  Search, Filter, XCircle, Activity, Zap, Briefcase, Flag
} from "lucide-react";
...
function StatusBadge({ status }: { status: string }) {
  ...
  const styles: Record<string, string> = {
    ...
    under_investigation: "bg-amber-500/10 text-amber-600 border-amber-500/20 dark:bg-amber-500/10 dark:text-amber-400 dark:border-amber-500/20",
    active_alert: "bg-red-500/10 text-red-600 border-red-500/20 dark:bg-red-500/10 dark:text-red-400 dark:border-red-500/20",
  ...
  const labels: Record<string, string> = { 
    pending: "Pending", in_progress: "In Progress", under_review: "Under Review", under_investigation: "Under Investigation",
    active_alert: "Active Alert", matched: "Matched", resolved: "Resolved", claimed: "Claimed", 
    approved: "Approved", confirmed: "Confirmed", completed: "Completed", cancelled: "Cancelled", rejected: "Rejected" 
  };
  ...
  return (
    <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold border capitalize ${style}`}>
      <span className={`w-1.5 h-1.5 rounded-full mr-1.5 ${
        cleanStatus === 'resolved' || cleanStatus === 'claimed' || cleanStatus === 'approved' || cleanStatus === 'confirmed' || cleanStatus === 'completed' ? 'bg-emerald-500' : 
        cleanStatus === 'in_progress' || cleanStatus === 'matched' || cleanStatus === 'under_review' || cleanStatus === 'under_investigation' ? 'bg-blue-500 animate-pulse' : 
        cleanStatus === 'active_alert' ? 'bg-red-500 animate-pulse' : 
        'bg-amber-500'
      }`} />
      {label}
    </span>
...
// ==========================================
function ActivityFeed({ complaints, foundItems, consultations, claims, flaggedItems }: { complaints: any[]; foundItems: any[]; consultations: any[]; claims: any[]; flaggedItems: any[] }) {
        ...
        <AnimatePresence initial={false}>
          {recentActivities.length === 0 ? (
            ...
            recentActivities.map((activity, index) => {
              ...
              else if (activity.type === 'flagged') { icon = <Flag className="h-4 w-4" />; colorClass = "bg-red-500/10 text-red-600 dark:text-red-400"; title = "New Item Flagged"; }
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-[#283113]/60 dark:text-[#F3FF74]/60 truncate mt-0.5">
                      {activity.tracking_id || activity.full_name || activity.title} • {activity.category || activity.service_type || activity.reason || 'Claim'}
                    </p>
                  </div>
...
export default function AdminDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<"complaints" | "found" | "consultations" | "claims" | "flagged">("complaints");
  
  const [complaints, setComplaints] = useState<any[]>([]);
  const [foundItems, setFoundItems] = useState<any[]>([]);
  const [consultations, setConsultations] = useState<any[]>([]);
  const [claims, setClaims] = useState<any[]>([]);
  const [flaggedItems, setFlaggedItems] = useState<any[]>([]); // ✅ NEW
  ...
  const fetchData = async () => {
    setLoading(true);
    const [compRes, foundRes, consultRes, claimsRes, flaggedRes] = await Promise.all([
      ...
      supabase.from("item_claims").select("*").order("created_at", { ascending: false }),
      supabase.from("flagged_items").select("*").order("created_at", { ascending: false }) // ✅ NEW
    ]);
    ...
    if (flaggedRes.data) setFlaggedItems(flaggedRes.data); // ✅ NEW
    setLoading(false);
  };
  ...
  const updateStatus = async (id: string, type: string, newStatus: string) => {
    ...
    if (!error) {
      ...
      else if (type === "flagged_items") setFlaggedItems(prev => prev.map(item => item.id === id ? { ...item, status: newStatus } : item)); // ✅ NEW
    }
    setUpdatingId(null);
  };
  ...
  const getActiveData = () => {
    switch (activeTab) {
      ...
      case "flagged": return flaggedItems; // ✅ NEW
      default: return complaints;
    }
  };

  const activeData = getActiveData();
  const tableType = activeTab === "complaints" ? "complaints" : activeTab === "found" ? "found_items" : activeTab === "consultations" ? "consultation_bookings" : activeTab === "claims" ? "item_claims" : "flagged_items"; // ✅ UPDATED
  ...
  const filteredData = useMemo(() => {
    return activeData.filter((item: any) => {
      ...
      const matchesSearch = searchQuery === "" ||
        ...
        item.title?.toLowerCase().includes(query) || // ✅ NEW for flagged
        item.reason?.toLowerCase().includes(query);  // ✅ NEW for flagged
  ...
  const activeAlerts = flaggedItems.filter(f => f.status === "Active Alert").length; // ✅ NEW
        ...
        <main className="container mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8 space-y-8">
          <div className="space-y-6">
            <div>
              ...
              <p className="text-[#283113]/60 dark:text-[#F3FF74]/60 mt-1">Monitor and manage all recovery cases, found items, consultations, claims, and flagged items.</p>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <StatCard title="Active Alerts" value={activeAlerts} icon={AlertTriangle} /> {/* ✅ NEW */}
              <StatCard title="Successfully Resolved" value={resolvedCases} icon={CheckCircle} trend="98% success rate" />
            </div>
          </div>

          {/* ✅ UPDATED: Full Width Table Section */}
          <div className="space-y-4">
            <div className="flex items-center gap-2 border-b border-[#283113]/10 dark:border-[#F3FF74]/10 overflow-x-auto no-scrollbar">
              ...
              {/* ✅ NEW: Flagged Items Tab */}
              <button onClick={() => { setActiveTab("flagged"); clearFilters(); }} className={`relative px-4 py-3 text-sm font-semibold transition-colors whitespace-nowrap ${activeTab === "flagged" ? "text-red-600 dark:text-red-400" : "text-[#283113]/50 dark:text-[#F3FF74]/50 hover:text-red-600 dark:hover:text-red-400"}`}>
                <span className="flex items-center gap-2"><Flag className="h-4 w-4" /> Flagged Items ({flaggedItems.length})</span>
                {activeTab === "flagged" && <motion.div layoutId="activeTab" className="absolute bottom-0 left-0 right-0 h-0.5 bg-red-500" />}
              </button>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 p-3 rounded-2xl border bg-white/80 border-[#283113]/10 dark:bg-[#283113]/60 dark:border-[#F3FF74]/10 backdrop-blur-sm">
              <div className="relative flex-grow">
                ...
                <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search by ID, Name, Title, or Reason..." className="w-full pl-9 pr-4 py-2.5 rounded-xl bg-white dark:bg-[#1A220D] border border-[#283113]/10 dark:border-[#F3FF74]/10 text-sm text-[#283113] dark:text-[#F3FF74] placeholder:text-[#283113]/40 dark:placeholder:text-[#F3FF74]/40 focus:outline-none focus:ring-2 focus:ring-[#F3FF74]/50 transition-all" />
              </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left text-sm">
                  <thead className="bg-[#283113]/5 dark:bg-[#F3FF74]/5 text-[#283113]/70 dark:text-[#F3FF74]/70 font-medium border-b border-[#283113]/10 dark:border-[#F3FF74]/10">
                    <tr>
                      {activeTab === "flagged" ? ( // ✅ NEW: Flagged Items Table Headers
                        <>
                  ...
                  <tbody className="divide-y divide-[#283113]/5 dark:divide-[#F3FF74]/5">
                    <AnimatePresence mode="wait">
                      {loading ? (
                        ...
                        <tr><td colSpan={6} className="px-6 py-12 text-center"><div className="flex flex-col items-center justify-center text-[#283113]/50 dark:text-[#F3FF74]/50"><AlertTriangle className="h-8 w-8 mb-2 text-[#283113]/30 dark:text-[#F3FF74]/30" /><p className="font-medium">No records found.</p></div></td></tr>
                      ) : (
                        filteredData.map((item: any, index: number) => (
                          <motion.tr key={item.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.03 }} className="group hover:bg-[#F3FF74]/5 dark:hover:bg-[#F3FF74]/5 transition-colors">
                            {activeTab === "flagged" ? ( // ✅ NEW: Flagged Items Table Rows
                              <>
                                <td className="px-6 py-4"><span className="font-mono font-semibold text-[#283113] dark:text-[#F3FF74]">{item.tracking_id}</span></td>
                                <td className="px-6 py-4 font-medium text-[#283113] dark:text-[#F3FF74]">{item.title}</td>
                                <td className="px-6 py-4 hidden md:table-cell text-sm text-[#283113]/70 dark:text-[#F3FF74]/70">{item.reason}</td>
                                <td className="px-6 py-4 hidden sm:table-cell text-sm text-[#283113]/60 dark:text-[#F3FF74]/60">{item.location}</td>
                              ...
                              <>
                                ...
                                <td className="px-6 py-4 hidden md:table-cell text-sm text-[#283113]/60 dark:text-[#F3FF74]/60">{item.preferred_date} at {item.preferred_time}</td>
                                <td className="px-6 py-4 hidden sm:table-cell"><div className="text-sm text-[#283113] dark:text-[#F3FF74]">{item.email}</div><div className="text-xs text-[#283113]/50 dark:text-[#F3FF74]/50">{item.phone}</div></td>
                                <td className="px-6 py-4"><StatusBadge status={item.status} /></td>
                                <td className="px-6 py-4 text-right">
                                  <div className="flex items-center justify-end gap-2">
                                    <button onClick={() => setSelectedItem(item)} className="p-1.5 rounded-lg bg-[#F3FF74]/20 dark:bg-[#F3FF74]/10 text-[#283113] dark:text-[#F3FF74] hover:bg-[#F3FF74]/40 transition-colors" title="View Details"><Eye className="h-4 w-4" /></button>
                              ...
                              <>
                                <td className="px-6 py-4"><span className="font-mono font-semibold text-[#283113] dark:text-[#F3FF74]">{item.tracking_id}</span></td>
                                <td className="px-6 py-4"><div className="font-medium text-[#283113] dark:text-[#F3FF74]">{item.full_name}</div></td>
                                <td className="px-6 py-4 hidden md:table-cell"><div className="text-[#283113] dark:text-[#F3FF74]">{item.email}</div><div className="text-xs text-[#283113]/50 dark:text-[#F3FF74]/50">{item.phone}</div></td>
                                ...
                                <td className="px-6 py-4 text-right">
                                  <div className="flex items-center justify-end gap-2">
                                    <button onClick={() => setSelectedItem(item)} className="p-1.5 rounded-lg bg-[#F3FF74]/20 dark:bg-[#F3FF74]/10 text-[#283113] dark:text-[#F3FF74] hover:bg-[#F3FF74]/40 transition-colors" title="View Details"><Eye className="h-4 w-4" /></button>
                                    <select value={item.status} onChange={(e) => updateStatus(item.id, tableType, e.target.value)} disabled={updatingId === item.id} className="bg-white dark:bg-[#283113] border border-[#283113]/20 dark:border-[#F3FF74]/20 text-[#283113] dark:text-[#F3FF74] text-xs rounded-lg px-3 py-1.5 focus:outline-none focus:ring-2 focus:ring-[#F3FF74]/50 disabled:opacity-50 cursor-pointer hover:border-[#F3FF74]/50 transition-colors">
                                      <option value="pending">Pending</option><option value="under_review">Under Review</option><option value="approved">Approved</option><option value="rejected">Rejected</option>
                                    </select>
                                  </div>
                                </td>
                              </>
                              ...
                              <>
                                <td className="px-6 py-4"><span className="font-mono font-semibold text-[#283113] dark:text-[#F3FF74]">{item.tracking_id}</span></td>
                                <td className="px-6 py-4"><div className="font-medium text-[#283113] dark:text-[#F3FF74]">{item.category || item.item_type}</div><div className="text-xs text-[#283113]/50 dark:text-[#F3FF74]/50 truncate max-w-[200px]">{activeTab === "complaints" ? item.item_type : item.description}</div></td>
                                <td className="px-6 py-4 hidden md:table-cell"><div className="text-[#283113] dark:text-[#F3FF74]">{item.is_anonymous ? "Anonymous" : item.full_name}</div><div className="text-xs text-[#283113]/50 dark:text-[#F3FF74]/50">{item.email}</div></td>
                                <td className="px-6 py-4 hidden sm:table-cell text-[#283113]/60 dark:text-[#F3FF74]/60">{new Date(item.created_at).toLocaleDateString()}</td>
                                <td className="px-6 py-4"><StatusBadge status={item.status} /></td>
                                <td className="px-6 py-4 text-right">
                                  <div className="flex items-center justify-end gap-2">
          ...
          {/* ✅ UPDATED: Moved Chart & Live Feed to bottom */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <DashboardChart complaints={complaints} foundItems={foundItems} />
            <ActivityFeed complaints={complaints} foundItems={foundItems} consultations={consultations} claims={claims} flaggedItems={flaggedItems} />
          </div>
        </main>
                ...
                <div className="sticky top-0 z-10 flex items-center justify-between px-6 py-4 border-b border-[#283113]/10 dark:border-[#F3FF74]/10 bg-[#FDFDF5]/90 dark:bg-[#283113]/90 backdrop-blur-md">
                  <div>
                    <h3 className="text-lg font-bold text-[#283113] dark:text-[#F3FF74]">
                      {activeTab === "consultations" ? "Consultation Details" : activeTab === "claims" ? "Claim Details" : activeTab === "flagged" ? "Flagged Item Details" : "Case Details"}
                    </h3>
                ...
                <div className="p-6 space-y-6">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs font-semibold uppercase tracking-wider text-[#283113]/50 dark:text-[#F3FF74]/50">
                      {activeTab === "complaints" ? "Complaint" : activeTab === "found" ? "Found Item" : activeTab === "consultations" ? "Consultation" : activeTab === "claims" ? "Item Claim" : "Flagged Item"}
                    </span>
                  </div>
                  
                  {activeTab === "flagged" ? ( // ✅ NEW: Flagged Item Details View
                    <>
                      <div className="space-y-2">
                        <h4 className="text-xs font-bold uppercase tracking-wider text-[#283113]/50 dark:text-[#F3FF74]/50">Reason for Flagging</h4>
                        <p className="text-sm text-[#283113] dark:text-[#F3FF74] leading-relaxed bg-white dark:bg-[#1A220D] p-4 rounded-xl border border-[#283113]/10 dark:border-[#F3FF74]/10">{selectedItem.reason}</p>
                      </div>
                      <div className="space-y-2">
                        <h4 className="text-xs font-bold uppercase tracking-wider text-[#283113]/50 dark:text-[#F3FF74]/50">Description</h4>
                        <p className="text-sm text-[#283113] dark:text-[#F3FF74] leading-relaxed bg-white dark:bg-[#1A220D] p-4 rounded-xl border border-[#283113]/10 dark:border-[#F3FF74]/10">{selectedItem.description}</p>
                      </div>
                      <div className="grid grid-cols-2 gap-4">
                        <div className="flex items-start gap-3 p-3 rounded-xl bg-white dark:bg-[#1A220D] border border-[#283113]/10 dark:border-[#F3FF74]/10">
                          ...
                          <div><p className="text-xs text-[#283113]/50 dark:text-[#F3FF74]/50 font-bold">Location</p><p className="text-sm text-[#283113] dark:text-[#F3FF74]">{selectedItem.location}</p></div>
                        </div>
                        <div className="flex items-start gap-3 p-3 rounded-xl bg-white dark:bg-[#1A220D] border border-[#283113]/10 dark:border-[#F3FF74]/10">
                          <Calendar className="h-5 w-5 text-[#283113]/60 dark:text-[#F3FF74]/60 flex-shrink-0 mt-0.5" />
                          <div><p className="text-xs text-[#283113]/50 dark:text-[#F3FF74]/50 font-bold">Date Flagged</p><p className="text-sm text-[#283113] dark:text-[#F3FF74]">{selectedItem.date_flagged}</p></div>
                        </div>
                      </div>
                    </>
                  ) : (
                        ...
                        {activeTab === "consultations" ? (
                          <>
                            <div className="flex items-start gap-3 p-3 rounded-xl bg-white dark:bg-[#1A220D] border border-[#283113]/10 dark:border-[#F3FF74]/10">
                              ...
                              <div><p className="text-xs text-[#283113]/50 dark:text-[#F3FF74]/50 font-bold">Service Type</p><p className="text-sm text-[#283113] dark:text-[#F3FF74]">{selectedItem.service_type}</p></div>
                            </div>
                            <div className="flex items-start gap-3 p-3 rounded-xl bg-white dark:bg-[#1A220D] border border-[#283113]/10 dark:border-[#F3FF74]/10">
                              <Calendar className="h-5 w-5 text-[#283113]/60 dark:text-[#F3FF74]/60 flex-shrink-0 mt-0.5" />
                              <div><p className="text-xs text-[#283113]/50 dark:text-[#F3FF74]/50 font-bold">Preferred Date & Time</p><p className="text-sm text-[#283113] dark:text-[#F3FF74]">{selectedItem.preferred_date} at {selectedItem.preferred_time}</p></div>
                            </div>
                          </>
                        ) : activeTab !== "consultations" && (
                          <>
                            <div className="flex items-start gap-3 p-3 rounded-xl bg-white dark:bg-[#1A220D] border border-[#283113]/10 dark:border-[#F3FF74]/10">
                              <MapPin className="h-5 w-5 text-[#283113]/60 dark:text-[#F3FF74]/60 flex-shrink-0 mt-0.5" />
                              <div><p className="text-xs text-[#283113]/50 dark:text-[#F3FF74]/50 font-bold">Location</p><p className="text-sm text-[#283113] dark:text-[#F3FF74]">{selectedItem.location || selectedItem.location_found}</p></div>
                            </div>
                            <div className="flex items-start gap-3 p-3 rounded-xl bg-white dark:bg-[#1A220D] border border-[#283113]/10 dark:border-[#F3FF74]/10">
                              <Calendar className="h-5 w-5 text-[#283113]/60 dark:text-[#F3FF74]/60 flex-shrink-0 mt-0.5" />
                              <div><p className="text-xs text-[#283113]/50 dark:text-[#F3FF74]/50 font-bold">Date</p><p className="text-sm text-[#283113] dark:text-[#F3FF74]">{selectedItem.incident_date || selectedItem.date_found}</p></div>
                            </div>
                          </>
                        )}
                        {!selectedItem.is_anonymous && (
                          <>
                            <div className="flex items-start gap-3 p-3 rounded-xl bg-white dark:bg-[#1A220D] border border-[#283113]/10 dark:border-[#F3FF74]/10">
                              ...
                              <div><p className="text-xs text-[#283113]/50 dark:text-[#F3FF74]/50 font-bold">Full Name</p><p className="text-sm text-[#283113] dark:text-[#F3FF74]">{selectedItem.full_name}</p></div>
                            </div>
                            <div className="flex items-start gap-3 p-3 rounded-xl bg-white dark:bg-[#1A220D] border border-[#283113]/10 dark:border-[#F3FF74]/10">
                              <Mail className="h-5 w-5 text-[#283113]/60 dark:text-[#F3FF74]/60 flex-shrink-0 mt-0.5" />
                              <div><p className="text-xs text-[#283113]/50 dark:text-[#F3FF74]/50 font-bold">Email</p><p className="text-sm text-[#283113] dark:text-[#F3FF74]">{selectedItem.email}</p></div>
                            </div>
                            {selectedItem.phone && (
                              <div className="flex items-start gap-3 p-3 rounded-xl bg-white dark:bg-[#1A220D] border border-[#283113]/10 dark:border-[#F3FF74]/10">
                                <Phone className="h-5 w-5 text-[#283113]/60 dark:text-[#F3FF74]/60 flex-shrink-0 mt-0.5" />
                                <div><p className="text-xs text-[#283113]/50 dark:text-[#F3FF74]/50 font-bold">Phone</p><p className="text-sm text-[#283113] dark:text-[#F3FF74]">{selectedItem.phone}</p></div>
                              </div>
                            )}
                          </>
                        )}
