...
function StatCard({ icon, endValue, suffix, label, delay }: { 
  ...
  const background = useMotionTemplate`radial-gradient(500px circle at ${smoothX}px ${smoothY}px, rgba(245, 158, 11, 0.08), transparent 40%)`;
  ...
  return (
    <motion.div
      ...
      className="group relative p-4 sm:p-8 rounded-2xl sm:rounded-3xl bg-white dark:bg-slate-900 backdrop-blur-sm border border-slate-200 dark:border-slate-800 hover:border-amber-500/30 transition-colors duration-300 overflow-hidden cursor-default shadow-sm hover:shadow-xl hover:shadow-amber-500/5"
    >
      <motion.div
      ...
      <div className="relative z-10 flex flex-col items-center text-center" style={{ transform: "translateZ(20px)" }}>
        <div className="p-2 sm:p-3 rounded-xl sm:rounded-2xl bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 mb-4 sm:mb-6 group-hover:scale-110 group-hover:border-amber-500/50 group-hover:shadow-lg group-hover:shadow-amber-500/10 transition-all duration-500">
          <div className="text-amber-600 dark:text-amber-400">{icon}</div>
        </div>
        
        <div className="flex items-baseline gap-1 mb-1 sm:mb-2">
          <span className="text-2xl sm:text-4xl md:text-5xl font-extrabold text-slate-900 dark:text-white tracking-tight">
        ...
        <p className="text-[10px] sm:text-sm md:text-base text-slate-500 dark:text-slate-400 font-medium uppercase tracking-wider">
          {label}
        </p>
      </div>
    </motion.div>
...
export function StatsCounter() {
  ...
  return (
    <section className="relative py-16 sm:py-24 px-3 sm:px-4 bg-white dark:bg-slate-950 transition-colors duration-300 overflow-hidden">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[400px] sm:w-[800px] h-[400px] sm:h-[800px] bg-amber-500/5 dark:bg-amber-500/5 rounded-full blur-[100px] sm:blur-[150px] pointer-events-none" />
      ...
      <div className="container mx-auto max-w-6xl relative z-10">
        ...
        >
          <span className="inline-block px-3 sm:px-4 py-1 sm:py-1.5 rounded-full bg-amber-500/10 text-amber-700 dark:text-amber-400 text-xs sm:text-sm font-semibold mb-3 sm:mb-4 border border-amber-500/20">
          ...
          <h2 className="text-2xl sm:text-3xl md:text-5xl font-extrabold text-slate-900 dark:text-white tracking-tight mb-3 sm:mb-4">
            {t("Trusted by", "Trusted by")} <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-500 to-yellow-600">{t("Thousands", "Thousands")}</span>
          </h2>
        ...
        >
          <div className="text-center mb-6 sm:mb-8">
            <p className="text-[10px] sm:text-xs font-bold uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500">
          ...
          <div className="relative w-full overflow-hidden">
            ...
            <div className="absolute left-0 top-0 bottom-0 w-12 sm:w-24 bg-gradient-to-r from-white dark:from-slate-950 to-transparent z-10 pointer-events-none" />
            <div className="absolute right-0 top-0 bottom-0 w-12 sm:w-24 bg-gradient-to-l from-white dark:from-slate-950 to-transparent z-10 pointer-events-none" />
            ...
            >
              {marqueeItems.map((partner, index) => (
                <div 
                  key={index}
                  className="flex items-center gap-2 sm:gap-3 px-3 sm:px-6 py-3 sm:py-4 rounded-xl sm:rounded-2xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-amber-500/30 hover:bg-amber-50 dark:hover:bg-amber-900/10 transition-all duration-300 group cursor-default"
                >
                  <div className="text-slate-400 dark:text-slate-500 group-hover:text-amber-600 dark:group-hover:text-amber-400 transition-colors duration-300">
                    {partner.icon}
                  </div>
                  <span className="text-xs sm:text-sm font-semibold text-slate-600 dark:text-slate-400 group-hover:text-slate-900 dark:group-hover:text-white transition-colors duration-300 whitespace-nowrap">
                    {partner.name}
                  </span>
                </div>
              ))}
